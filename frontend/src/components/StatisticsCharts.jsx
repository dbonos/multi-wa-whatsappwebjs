...
export function ResponseTimeChart({ data, periods }) {
  const chartData = periods.map((period, index) => {
    ...
    return {
      ...
      'Previous Message': stat.previous_message?.avg_response_time_minutes || 0,
    };
  });
    ...
    <div className="card p-4">
      ...
      <ResponsiveContainer width="100%" height={300}>
        <LineChart data={chartData}>
          ...
          <Line type="monotone" dataKey="Previous Message" stroke="#128C7E" strokeWidth={2} />
        </LineChart>
      </ResponsiveContainer>
    </div>
  );
...
export function CustomerCountChart({ data, periods }) {
  const chartData = periods.map((period, index) => {
    ...
    return {
      ...
      'Previous Message': stat.previous_message?.count || 0,
      'New Customer (Tidak Dibalas)': stat.new_customer?.unreplied_count || 0,
      'Previous Message (Tidak Dibalas)': stat.previous_message?.unreplied_count || 0,
    };
  });
    ...
    <div className="card p-4">
      ...
      <ResponsiveContainer width="100%" height={300}>
        <BarChart data={chartData}>
          ...
          <Bar dataKey="Previous Message" fill="#128C7E" />
          <Bar dataKey="New Customer (Tidak Dibalas)" fill="#FF6B6B" />
          <Bar dataKey="Previous Message (Tidak Dibalas)" fill="#FF8787" />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
...
export function PeriodComparisonChart({ data, periods }) {
  const chartData = periods.map((period, index) => {
    ...
    return {
      ...
      'Previous Message (min)': stat.previous_message?.avg_response_time_minutes || 0,
      'New Customer Count': stat.new_customer?.count || 0,
      'Previous Message Count': stat.previous_message?.count || 0,
      'New Customer (Tidak Dibalas)': stat.new_customer?.unreplied_count || 0,
      'Previous Message (Tidak Dibalas)': stat.previous_message?.unreplied_count || 0,
    };
  });
    ...
    <div className="card p-4">
      ...
      <ResponsiveContainer width="100%" height={400}>
        <BarChart data={chartData}>
          ...
          <Bar yAxisId="left" dataKey="Previous Message (min)" fill="#128C7E" />
          <Bar yAxisId="right" dataKey="New Customer Count" fill="#34B7F1" />
          <Bar yAxisId="right" dataKey="Previous Message Count" fill="#075E54" />
          <Bar yAxisId="right" dataKey="New Customer (Tidak Dibalas)" fill="#FF6B6B" />
          <Bar yAxisId="right" dataKey="Previous Message (Tidak Dibalas)" fill="#FF8787" />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
