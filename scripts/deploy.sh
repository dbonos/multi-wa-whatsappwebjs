...
for MIG in database/migrations/create_statistics_tables.sql database/migrations/convert_timestamp_to_datetime_wib.sql database/migrations/add_attachment_url_to_messages.sql database/migrations/add_public_domain_to_sessions.sql database/migrations/create_user_menu_permissions.sql database/migrations/create_activity_logs_table.sql; do
  if [ -f "$MIG" ]; then
